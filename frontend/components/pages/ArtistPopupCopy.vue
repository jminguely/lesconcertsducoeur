<template>
  <div
    ...
    class="fixed top-0 left-0 flex flex-col w-screen h-screen px-10 py-10 overflow-auto duration-300 ease-in-out bg-white opacity-0 lg:overflow-hidden"
  >
    <div>
      <button class="focus:outline-none" @click="$emit('update:popup', false)">
        ...
        >
          <g
            ...
            transform="translate(-193.793 -96.793)"
          >
            <line
              id="Line_21"
              data-name="Line 21"
              x2="30.425"
              y2="30.425"
              transform="translate(194.5 97.5)"
              fill="none"
              stroke="#4d9a70"
              stroke-width="2"
            />
            <line
              ...
              x2="30.425"
              y2="30.425"
              transform="translate(224.925 97.5) rotate(90)"
              fill="none"
              stroke="#4d9a70"
              stroke-width="2"
            />
    ...
    <div class="flex flex-col justify-between py-10 lg:flex-row">
      <div class="absolute flex w-1/5 right-5 lg:flex-col lg:static top-4">
        <button
          class="flex my-2 mr-6 text-xl focus:outline-none lg:mr-0"
          @click="goBack()"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          >
            <g
              ...
              transform="translate(-193.793 -66.793)"
            >
              <line
                id="Line_21"
                data-name="Line 21"
                x2="16.301"
                y2="16.301"
                transform="translate(194.5 83.573)"
                fill="none"
                stroke="#4d9a70"
                stroke-width="2"
              />
              <line
                ...
                x2="16.301"
                y2="16.301"
                transform="translate(210.801 67.5) rotate(90)"
                fill="none"
                stroke="#4d9a70"
                stroke-width="2"
              />
          ...
          <template v-if="artists[previousIndex] != null">
            <span class="pl-3 hidden lg:block hover:text-gray-500">
              {{ artists[previousIndex].first_name }}
              {{ artists[previousIndex].last_name }}
            </span>
          </template>
        </button>
        ...
        <button class="flex my-2 text-xl focus:outline-none" @click="next()">
          ...
          >
            ...
            >
              ...
              <line
                ...
                y1="16.301"
                x2="16.301"
                transform="translate(16.301 0) rotate(90)"
                fill="none"
                stroke="#4d9a70"
                stroke-width="2"
              />
          ...
          <template v-if="artists[previousIndex] != null">
            <span class="pl-3 hidden lg:block hover:text-gray-500">
              {{ artists[nextIndex].first_name }}
              {{ artists[nextIndex].last_name }}
            </span>
          </template>
        </button>
      </div>
      ...
      <div
        class="mb-6 overflow-auto lg:w-2/5 lg:mb-0"
        style="max-height: calc(100vh - 125.83px)"
      >
        <h1 class="pb-4 text-6xl">
        ...
        <p class="my-6 prose-xl">
          Vous souhaitez booker ce musicien·nne dans votre établissement?
        </p>
        <nuxt-link class="prose-xl text-green-500 no-underline" to="/contact"
          >> Contact</nuxt-link
        >
      </div>

      <div
        class="lg:overflow-auto lg:w-2/6"
        style="max-height: calc(100vh - 125.83px)"
      >
        <div class="ml-auto">
          <template v-if="selected.picture != null">
            <img
              class="object-cover h-64 lg:w-120 lg:h-128 w-96"
              :src="
...
export default {
  ...
  computed: {
    ...
    previousIndex() {
      return (
        (this.currentIndex === 0 ? this.artists.length : this.currentIndex) - 1
      )
    },
    nextIndex() {
      return this.currentIndex === this.artists.length - 1
        ? 0
        : this.currentIndex + 1
    },
  },
